...
class MapperInit(MapperBase):
    ...
    def getSysPower(self, timeGraph):
        ...
        sysPower = 0
        self.powerlevel[0] = 3000 * 10 ** (-12) # P2P energy consumption
        self.powerlevel[1] = 3000 * 10 ** (-12) # P2C energy consumption
        self.powerlevel[2] = 5000 * 10 ** (-10) # C2C energy consumption
        for node in allNodes:
            ...
            intype = timeGraph.nodes[node]['tinx']
            outtype = timeGraph.nodes[node]['toutx']
            Acc = self.getAccbyName(accName, self.AccList)

            # sysPower = sysPower + (end - start) * Acc.power

            sysPower = sysPower + (end - start - comm) * Acc.power + (timeGraph.nodes[node]['tin'] *
                                                                      self.powerlevel[intype] + timeGraph.nodes[node]['tout']
                                                                      * self.powerlevel[outtype]) * (Acc.NetBand * (10 ** (9)) * 8) / Acc.Dtype
...
#                     edition2
    ...
    def getModalTune(self, node):
        ...
        for pred in preds:
            ...
            if self.GtimeReserve.nodes[node]['acc'] != self.GtimeReserve.nodes[pred]['acc'] \
                ...
                elif self.PlatformElapsed == self.getAccMaxElapsed(self.GaccLastTuneTmp) and self.syspower > self.getSysPower(self.GtimeLastTuneTmp):
                    self.GaccReserve = copy.deepcopy(self.GaccReserveTmp)
                    self.GtimeReserve = copy.deepcopy(self.GtimeReserveTmp)
                    self.AccLayersReserve = copy.deepcopy(self.AccLayersReserveTmp)
                    # update the finalized mapping
    ...
    def getHomoNeighbor(self):
        ...
        power = self.getSysPower(self.GtimeLastTune)
...
if __name__ == "__main__":
    ...
    for Modality in Modalities:
        for AccsList in AccsLists:
            ...
            # caution: H2H doesn't support multilevel
            # need to be comment out when searching multilevel mapping
            InitMapper.getModalityLayers(Modality)
            InitMapper.getAccList(AccsList)
